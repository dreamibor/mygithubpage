...
def jaccard_sim(v0,v1):
  ...
  #You will need to replace with the real function
  c = set(v0.keys()) & set(v1.keys())
  d = set(v0.keys()).union(set(v1.keys()))
  if not c:
    return 0
  L_v0 = sum(min(v0.get(sk), v1.get(sk)) for sk in c)
  L_v1 = sum(max(v0.get(sk), v1.get(sk)) for sk in d)
  return float(L_v0)/float(L_v1)
...
def dice_sim(v0,v1):
  ...
  if not c:
    return 0
  L_v0 = sum(min(v0.get(sk), v1.get(sk)) for sk in c)
  L_v1 = sum(v0.values()+v1.values())
  return float(2*L_v0)/float(L_v1)
...
def most_word(N):
    word_list = []
    n_counts = sorted(o_counts.iteritems(), key = lambda x: x[0], reverse = False)
    for i in n_counts[:N]:
        word_list.append(wid2word[i[0]])
    return word_list

def less_word(N):
    word_list = []
    n_counts = sorted(o_counts.iteritems(), key = lambda x: x[0], reverse = True)
    for i in n_counts[:N]:
        word_list.append(wid2word[i[0]])
    return word_list
...
def print_sorted_pairs(similarities, o_counts, first=0, last=100):
  ...
  for pair in sorted(similarities.keys(), key=lambda x: similarities[x], reverse = True)[first:last]:
    #if wid2word[pair[0]] == 'phone' or wid2word[pair[1]] == 'phone':
...
def Pearson(wid_pairs, counts, method):
    y = [method(vectors[wid0],vectors[wid1]) for (wid0,wid1) in wid_pairs]
    x = [counts[wid0][wid1] if wid1 in co_counts[wid0] else 0 for (wid0,wid1) in wid_pairs]
    #x = [abs(counts[wid0]-counts[wid1]) for (wid0,wid1) in wid_pairs]
    # x_sum = sum(x)/len(x)
    # print(x_sum)
    #plt.scatter(x,y)
    #plt.show()
...
#test_words = ["computer", "laptop", "pc","mouse","mac","cpu"]
#test_words = ["speed", "brain","memory","workstation","run","data"]
test_words = ["data","shoes", "sky","classic","drink","human"]
#test_words = ["make", "hope","sister", "love", "meet","today"]
#test_words = ["filter","spider", "formula","buffer","scar","soar"]

# random_words = random.sample(word2wid.keys(),45)
...
print(Pearson(wid_pairs, co_counts, cos_sim))
print(Pearson(wid_pairs, co_counts, jaccard_sim))
print(Pearson(wid_pairs, co_counts, dice_sim))
